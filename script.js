...
window.onload = fetchData("https://public.opendatasoft.com/api/records/1.0/search/?dataset=us-colleges-and-universities&q=&facet=city&facet=state&facet=naics_desc&refine.state=MA");
var datum = [];
...
function displayData(Universities) {
	return Universities.map(function (University, index) {
		datum.push(University);
		...
		a = createNode("a");
		addBtn = createNode("a");
		img.src = University.picture.medium;
		strong.innerHTML = `${University.name}`;
		span.innerHTML = `${University.dob.age} ${University.gender}`;
		a.innerHTML = `${University.email}`;
		a.setAttribute("href", `${University.email}`);
		address.innerHTML = `${University.location.city}, ${University.location.state} , ${University.location.country}, ${University.location.postcode} `;
		append(figure, img);
		append(li, figure);
		append(li, strong);
		append(li, span);
...
function prepareList(University){
	var results = "";
	results = `<li><figure><img src=${University.picture.medium} /></figure>\n
